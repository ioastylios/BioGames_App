...
public class Walk extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 4; i++)
        {
            final int resourceId = resources.getIdentifier("walk"+(i+1)+"_1", "id", context.getPackageName());
            final int index = i+1;
            mEasy[i] = findViewById(resourceId);
            mEasy[i].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    passQuizTypeAndDifficulty("walk"+(index)+"_1");
                }
            });
        }
        for (int i = 0; i < 4; i++)
        {
            final int resourceId = resources.getIdentifier("walk"+(i+1)+"_2", "id", context.getPackageName());
            final int index = i+1;
            mMedium[i] = findViewById(resourceId);
            mMedium[i].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    passQuizTypeAndDifficulty("walk"+(index)+"_2");
                }
            });
        }
        for (int i = 0; i < 4; i++)
        {
            final int resourceId = resources.getIdentifier("walk"+(i+1)+"_3", "id", context.getPackageName());
            final int index = i+1;
            mHard[i] = findViewById(resourceId);
            mHard[i].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    passQuizTypeAndDifficulty("walk"+(index)+"_3");
                }
            });
        }
        for (int i = 0; i < 4; i++)
        {
            final int resourceId = resources.getIdentifier("walk"+(i+1)+"_4", "id", context.getPackageName());
            final int index = i+1;
            mHardcore[i] = findViewById(resourceId);
            mHardcore[i].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    passQuizTypeAndDifficulty("walk"+(index)+"_4");
                }
            });
        }
    ...
    public void onResume(){
        ...
        for (int i = 0; i < 4; i++)
        {
            String Key = "walk"+(i+1)+"_2";
            checkGameState(Key,mMedium[i]);
        }
        for (int i = 0; i < 4; i++)
        {
            String Key = "walk"+(i+1)+"_3";
            checkGameState(Key,mHard[i]);
        }
        for (int i = 0; i < 4; i++)
        {
            String Key = "walk"+(i+1)+"_4";
...
//        if (!quizTypeAndDiff.equals("walk4_1")) {
            Intent i = new Intent(this, WalkGamesActivity.class);
